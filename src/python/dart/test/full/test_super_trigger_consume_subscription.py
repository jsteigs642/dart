...
class TestSuperTriggerConsumeSubscription(unittest.TestCase):
    def setUp(self):
    ...
    def test_super_trigger_consume_subscription(self):
        ...
        tr = Trigger(data=TriggerData('test-trigger', 'subscription_batch', None, tr_args, TriggerState.ACTIVE))
        self.trigger = self.dart.save_trigger(tr)

        st_args = {'fire_after': 'ALL', 'completed_trigger_ids': [self.trigger.id]}
        st = Trigger(data=TriggerData('test-super-trigger', 'super', [self.workflow.id], st_args, TriggerState.ACTIVE))
        super_trigger = self.dart.save_trigger(st)
        ...
        self.dart.delete_trigger(super_trigger.id)
