angular
    ...
    .directive('dtWorkflowsTable', [function() {
        return {
            restrict: 'E',
            scope: {
                options: '='
            },
            templateUrl: 'static/partials/directives/tables/workflows.html',
            controller: ['$scope', 'WorkflowService', 'EntityModalService', 'FilterService', '$state',
                function($scope, WorkflowService, EntityModalService, FilterService, $state) {
                    $scope.$state = $state;
                    ...
                    $scope.onPaginationChange = function(page, limit) {
                        ...
                        $scope.$p = WorkflowService.getEntities(limit, (page - 1) * limit, $scope.filters.concat($scope.hiddenFilters));
                    ...
                    $scope.showEntityModal = function(ev, entity, mode) {
                        ...
                        var saveFunc = null;
                        if (mode === 'edit') {
                            saveFunc = function (entity) { return WorkflowService.updateEntity(entity).then(function(e) { entity = e; return e }) }
                        }
                        if (mode === 'duplicate') {
                            saveFunc = function (entity) { return WorkflowService.saveEntity(entity) }
                        }
                        EntityModalService.showDialog(ev, e,
                            function () {
                                return WorkflowService.getSchema().then(function(response) {
                                    if (mode === 'view') {
                    ...
                    $scope.manuallyRunWorkflow = function(e) {
                        WorkflowService.manuallyRunWorkflow(e.id)
                    };
                    WorkflowService.getSchema().then(function(response) {
